...
export async function visitarComoAdmin(page: Page, baseUrl: string, credencial: CredencialAgente, outputDir: string): Promise<void> {
  ...
  await verificarSemErroVisivel(page, outputDir, credencial.papel, "/admin/professores");

  await page.goto(`${baseUrl}/admin/financeiro/configuracao`);
  await verificarSemErroVisivel(page, outputDir, credencial.papel, "/admin/financeiro/configuracao");

  await page.goto(`${baseUrl}/auditoria`);
  await verificarSemErroVisivel(page, outputDir, credencial.papel, "/auditoria");
}
