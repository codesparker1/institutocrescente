...
export default async function DevedoresPage({ searchParams }: DevedoresPageProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-bold text-texto">Lista de Devedores</h1>
        <p className="text-sm text-texto-suave">Alunos com propinas ou multas em atraso além do período de tolerância.</p>
      </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex overflow-hidden rounded-lg border border-navy-100 text-xs font-medium">
                <Link
                  ...
                  className={`px-3 py-1.5 ${ordenacao === "antiguidade" ? "bg-navy-700 text-gold-100" : "bg-white text-texto hover:bg-navy-50"}`}
                >
                  Antiguidade
                </Link>
                <Link
                  href={`?${new URLSearchParams({ ...Object.fromEntries(filtrosQuery), sort: "valor" })}`}
                  className={`px-3 py-1.5 ${ordenacao === "valor" ? "bg-navy-700 text-gold-100" : "bg-white text-texto hover:bg-navy-50"}`}
                >
                  Valor
                </Link>
                <Link
                  href={`?${new URLSearchParams({ ...Object.fromEntries(filtrosQuery), sort: "nome" })}`}
                  className={`px-3 py-1.5 ${ordenacao === "nome" ? "bg-navy-700 text-gold-100" : "bg-white text-texto hover:bg-navy-50"}`}
                >
                  Nome
                </Link>
              </div>
              <a
                ...
                className="flex items-center gap-1.5 rounded-lg border border-navy-100 bg-white px-3 py-1.5 text-xs font-medium text-texto hover:bg-navy-50"
              >
                <Printer size={14} />
                Imprimir
              </a>
            ...
            <Table>
              ...
              <Tbody>
                {devedores.map((d) => (
                  <Tr key={d.alunoId}>
                    <Td>
                      <Link href={`/alunos/${d.alunoId}`} className="font-medium text-texto hover:underline">
                        {d.nome}
                      </Link>
                      <p className="text-xs text-texto-suave">{d.numeroEstudante}</p>
                    </Td>
                    <Td>
                      {d.curso} · {d.anoCurricular}º Ano
                    </Td>
                    <Td>{CATEGORIA_LABEL[d.categoria]}</Td>
                    <Td className="font-semibold text-texto">{formatCurrency(d.valorEmDivida)}</Td>
                    <Td>
