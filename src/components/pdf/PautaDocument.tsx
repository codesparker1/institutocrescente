...
const styles = StyleSheet.create({
  ...
  cellNum: { width: "5%" },
  cellEstudante: { width: "13%" },
  cellNome: { width: "24%" },
  cellEpoca: { width: "9%", textAlign: "center" },
  cellEstado: { width: "13%" },
  cellFinal: { width: "9%", textAlign: "center" },
  headerCell: {
    padding: 6,
    fontSize: 7,
    fontWeight: 700,
...
export interface PautaDocumentProps {
        ...
        <View fixed>
          <View style={styles.header}>
            ...
            <View style={styles.headerInfo}>
              ...
              <Text style={styles.institutionSubtitle}>Direcção Académica | Pauta de Notas</Text>
            </View>
          </View>
          <View style={styles.headerDivider} />

          <View style={styles.titleBar}>
            <Text style={styles.titleBarText}>PAUTA DE NOTAS</Text>
          </View>
        ...
        <View style={styles.table}>
          ...
          {alunos.map((aluno) => (
            <View key={aluno.numeroEstudante} style={styles.tableRow} wrap={false}>
              ...
              <Text style={[styles.cell, styles.cellNome]}>
                {aluno.nome}
                {aluno.tentativa > 1 ? ` (${aluno.tentativa}ª tentativa)` : ""}
              </Text>
              {EPOCA_ORDEM.map((epoca) => (
                <Text key={epoca} style={[styles.cell, styles.cellEpoca]}>
                  {aluno.notasPorEpoca[epoca] ?? "—"}
                </Text>
              ))}
              <Text style={[styles.cell, styles.cellEstado]}>{ESTADO_LABEL[aluno.estado]}</Text>
              <Text style={[styles.cell, styles.cellFinal]}>{aluno.notaFinal !== null ? aluno.notaFinal.toFixed(1) : "—"}</Text>
            </View>
          ))}
        </View>
      </Page>
