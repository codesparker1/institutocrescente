    ...
    <div className="flex flex-col gap-4">
          ...
          {documentos.map((doc) => (
            <li key={doc.id} className="flex items-center justify-between gap-3 border-b border-navy-50 pb-2 last:border-0 last:pb-0">
              <div className="flex flex-col">
                <a href={`/api/documentos/${doc.id}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-navy-800 hover:underline">
                  {doc.nome}
                </a>
      ...
      <form action={formAction} className="flex flex-wrap items-end gap-2 border-t border-navy-50 pt-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-navy-500">Descrição</label>
          <Input type="text" name="nome" required placeholder="Ex.: Certificado de transferência" className="w-56" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-navy-500">Ficheiro (PDF, JPG, PNG — máx. 10MB)</label>
